...
clear all
clc
t_final = 1000;           % final simulation time (sec)
t_rudderexecute = 1;    % time rudder is executed (sec)
h = 1;                 % sampling time (sec)

disp('10-10 Zigzag maneuver for the Mariner class vessel')

% 20-20 zigzag maneuver for the Mariner class cargo ship
% cruise speed U0 = 7.7 m/s (see mariner.m)
x111zz=[0,0,0,0,-50,0,0];
x  = x111zz';   % x  = [ u v r x y psi delta ]' (initial values)
ui = 0;            % delta_c = 0  for time t < t_rudderexecute
 
[t,u,v,r,x,y,psi,U] = Mariner_ZigZag('mariner',x,ui,t_final,t_rudderexecute,h,[10,10]);
